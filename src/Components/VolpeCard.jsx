...
import { useData } from '../Context/DataContext'; // <--- 1. DataContext-i import edirik
import './css/VolpeCard.css';

// FoxIcon SVG (Dəyişmədi)
const FoxIcon = () => (
...
const VolpeCard = ({ color = 'orange' }) => {
  ...
  const { cards } = useData(); // <--- 2. Kartları çağırırıq

  // 3. Kartların balansını toplayırıq (useMemo performans üçündür)
  const displayBalance = useMemo(() => {
    // Əgər kart yoxdursa 0 qaytar
    if (!cards || !Array.isArray(cards) || cards.length === 0) {
      return "0.00";
    }

    // Bütün kartların balansını topla (AZN ilə)
    const totalInAZN = cards.reduce((sum, card) => {
      return sum + (Number(card.balance) || 0);
    }, 0);

    // Seçilmiş valyutaya çevir və formatla
    return convertAmount(totalInAZN).toFixed(2);
    
  }, [cards, convertAmount]); // Kartlar və ya valyuta dəyişəndə yenidən hesabla
